...
import 'package:geo_assistant/Religious/andrew.dart';
import 'package:geo_assistant/Religious/badshah.dart';
import 'package:geo_assistant/Religious/datadarbar.dart';
import 'package:geo_assistant/Religious/jamia.dart';
import 'package:geo_assistant/Religious/sacred.dart';
import 'package:geo_assistant/Religious/wazir.dart';
import 'package:geo_assistant/Religious/babashahjamal.dart';
import 'package:geo_assistant/Religious/begumshahimosque.dart';
import 'package:geo_assistant/Religious/bibipakdaman.dart';
import 'package:geo_assistant/Religious/cathedral.dart';
import 'package:geo_assistant/Religious/madholal.dart';
import 'package:geo_assistant/Religious/shuhada.dart';
import 'package:geo_assistant/Religious/mianmir.dart';
import 'package:geo_assistant/Religious/neevin.dart';
import 'package:geo_assistant/Religious/sunehri.dart';
...
class _ReligiousState extends State<Religious> {
  ...
  List<Place> _religiousPlaces = [];
  bool _showAllReligiousPlaces = false;
  ...
  Future<void> _initLocation() async {
    ...
    GeofenceService().startListening((newCity) {
      if (mounted) {
        ...
        _fetchReligiousPlaces();
      }
    });
  }

  Future<void> _fetchReligiousPlaces() async {
    try {
      final url = Uri.parse(
          '${LocationService.baseUrl}/places/?city=$_detectedCity&category=Religious Place');
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            _religiousPlaces = data.map((x) => Place.fromJson(x)).toList();
            _isLoading = false;
          });
        }
      } else {
        throw Exception("Failed to load religious places");
      }
    } catch (e) {
      print("[Fetch Religious Error] $e");
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            "To discover the top-rated religious places, hotels, and restaurants in your current city, please enable Location Services and allow GPS access.",
            style: TextStyle(
  ...
  Widget _buildReligiousGrid(double screenWidth) {
    if (_isLoading) {
      return const Center(
        child: Padding(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Text(
                "No religious places found in $_detectedCity.",
    ...
    final displayList = _showAllReligiousPlaces ? _religiousPlaces : _religiousPlaces.take(7).toList();
    ...
    return Column(
      children: [
        Wrap(
          ...
          children: displayList.map((place) {
        return Padding(
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      print(
                          '[OpenPlace] name=${place.name} imageUrl=${place.imageUrl} raw=${place}');
                      Navigator.push(
                        context,
                        MaterialPageRoute(
    ...
    if (!_showAllReligiousPlaces && _religiousPlaces.length > 7)
      Padding(
        padding: const EdgeInsets.symmetric(vertical: 20.0),
        child: ElevatedButton.icon(
          onPressed: () {
            setState(() {
              _showAllReligiousPlaces = true;
            });
          },
          icon: const Icon(Icons.expand_more, color: Color(0xFF091F2C)),
          label: const Text(
            "View More Religious Places",
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildReligiousGrid(screenWidth),
                const SizedBox(height: 30),
              ],
            ),
          ),
