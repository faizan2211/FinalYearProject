...
import 'package:geo_assistant/chatbot/chat_bot.dart';
...
import 'package:geo_assistant/MainPage/favorites_screen.dart';
...
class _HomeState extends State<Home> {
  ...
  Future<void> _initLocation() async {
    ...
    if (pos != null) {
      final city = await locService.getCityFromCoords(pos.latitude, pos.longitude);
      final country = await locService.getCountryFromCoords(pos.latitude, pos.longitude);
      setState(() {
        _detectedCity = city;
  ...
  Future<void> _fetchRestaurants() async {
    try {
      final url = Uri.parse('${LocationService.baseUrl}/places/?city=$_detectedCity&category=Restaurant');
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            _restaurants = data.map((x) => Place.fromJson(x)).toList();
            _isLoading = false;
          });
        }
      } else {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            "To discover the top-rated restaurants, hotels, and tourist attractions in your current city, please enable Location Services and allow GPS access.",
            style: TextStyle(
  ...
  Widget _buildRestaurantsGrid(double screenWidth) {
    if (_isLoading) {
      return const Center(
        child: Padding(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Text(
                "No top-rated restaurants found in $_detectedCity.",
    ...
    final displayList = _showAllRestaurants ? _restaurants : _restaurants.take(7).toList();
    ...
    return Column(
      children: [
        Wrap(
          ...
          children: displayList.map((restaurant) {
            return Padding(
              ...
              child: Container(
                ...
                child: Stack(
                  children: [
                    (restaurant.imageUrl.isNotEmpty && restaurant.imageUrl.startsWith('http'))
                        ? Image.network(
                            restaurant.imageUrl,
                      ...
                      child: GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => getInfoPage(restaurant),
                            ),
                          );
                        },
                        child: Container(
        ...
        if (!_showAllRestaurants && _restaurants.length > 7)
          Padding(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Geo Assistant'),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    child: TextField(
                      onSubmitted: (value) async {
                        if (value.trim().isNotEmpty) {
                          try {
                            final url = Uri.parse('${LocationService.baseUrl}/places/?city=$_detectedCity');
                            final response = await http.get(url, headers: {'ngrok-skip-browser-warning': 'true'});
                            if (response.statusCode == 200) {
                              final List<dynamic> data = json.decode(response.body);
                              final allPlaces = data.map((x) => Place.fromJson(x)).toList();
                              final place = allPlaces.firstWhere((p) => p.name.toLowerCase().contains(value.toLowerCase().trim()));
                              if (context.mounted) {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => getInfoPage(place)));
                              }
                            }
                          } catch (e) {
                            if (context.mounted) {
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => const malls()),
                          );
                        },
                        child: _categoryItem(
                          screenWidth,
                ...
                _buildRestaurantsGrid(screenWidth),
                const SizedBox(height: 30),
              ],
            ),
          ),
